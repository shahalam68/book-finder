...
export default function BookFinder() {
  ...
  const [isResetVisible, setResetVisibility] = useState(false);
  ...
  const handleSelectChange = (event) => {
    ...
    switch (newSelectOption) {
      case "name_asc":
        setAllBooks([...allBooks].sort((a, b) => a.title.localeCompare(b.title)));
        break;
      case "name_desc":
        setAllBooks([...allBooks].sort((a, b) => b.title.localeCompare(a.title)));
        break;
      case "year_asc":
              ...
              <div className="flex">
                <div className="relative w-full overflow-hidden rounded-lg border-2 border-[#1C4336] text-[#1C4336] md:min-w-[380px] lg:min-w-[440px]">
                  ...
                  <div className="absolute right-0 top-0 flex h-full items-center">
                    ...
                    <button
                      type="reset"
                      className={`mr-1.5 flex items-center space-x-1.5 rounded-md rounded-e-lg bg-[#1C4336] px-4 py-2.5 text-sm text-white ${isResetVisible ? '' : 'hidden'}`}
                    >
                      <svg
                        className="h-[14px] w-[14px]"
                        aria-hidden="true"
